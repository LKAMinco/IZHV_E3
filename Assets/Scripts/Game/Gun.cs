...
public class Gun : MonoBehaviour
{
   ...
   private Transform mBulletDirector;
    ...
    public void ShootGun(Transform director)
    {
        ...
        if (shotgun)
        {
            float rot = director.eulerAngles.y;
            Debug.Log(rot);
            for (int i = 0; i < shotgunBullets; i++)
            {
                Quaternion bulletRotation = Quaternion.Euler(director.eulerAngles.x, rot - (shotgunSpread * shotgunBullets / 2),
                    director.eulerAngles.z);
